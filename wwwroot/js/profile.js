    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (!result.errorMessage) {
                ...
                $(".inputObjectID").each(function () {
                    $(this).text(result.objectId);
                });
                $("#inputLastPasswordChangeDateTime").text(result.lastPasswordChangeDateTime);
                $(".inputCreatedDateTime").each(function () {
                    $(this).text(result.createdDateTime);
                });
    ...
    $.post("/api/userattributes", payload, function (result) {
        ...
        if (typeof result === "string" && result.trim() !== "") {
            try {
                ...
                result = {}; // fallback to avoid breaking the rest of the logic
            }
            }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (!result.errorMessage) {
                ...
                // Sign-in name
                $("#inputSignInEmail").val(result.singInEmail);  // <-- your C# property is "SingInEmail" (typo), so JSON is "singInEmail"
                $("#inputEmail").text(result.signInEmail);
                $("#inputSignInType").val(result.signintype);    // C# is "signintype", JSON is "signintype"
                $("#inputIssuer").val(result.issuer);            // C# is "Issuer", JSON is "issuer"
                $("#inputIssuerTemp").text(result.issuerTemp);   // C# is "IssuerTemp", JSON is "issuerTemp"
                ...
                if (result.signintype === "userPrincipalName") {
                    $("#EditSignIn").hide();
                    $("#SignInUpdate").hide();
                    // Disable the field so it cannot be modified
                    $("#inputSignInEmail").prop("disabled", true);
                }
                else {
                    // For other sign-in types, re-enable the field if MFA is satisfied
                    $("#inputSignInEmail").prop("disabled", !mfaFulfilled);
                }
